...
- Floating, fixed modal form shows up to customize the planet including: Name, Description, Distance from Sun (in KM), Size, Planetary Characteristics (pick list of various planetary characteristics that determine appearance of planet e.g. `rocky` and `oceans` may look similar to Earth)
...
- Prefer arrow functions (e.g. `const doThing = (arg) => {}`) over function declarations
